  ...
  for (let blur = 0; blur < 10; blur++) {
    ...
    for (let row = -10; row < rows + 10; row++) {
      for (let col = 0; col < columns; col++) {
        let x = (col + 0.5) * gridSize;
        let y = (row + 0.5) * gridSize;
...
function drawTriangle(x, y, size, rot) {
  ...
  const x2 = sin(0.333 * 2 * PI) * size;
  const y2 = cos(0.333 * 2 * PI) * size;
  const x3 = sin(0.666 * 2 * PI) * size;
  const y3 = cos(0.666 * 2 * PI) * size;
